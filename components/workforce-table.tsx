...
import { User, Bot, Cpu, UserMinus, CheckCircle, AlertCircle, ShieldCheck, ShieldAlert, ShieldOff } from "lucide-react"
import {
  Table,
  TableBody,
  TableCell,
...
import { workersWithExtensions as workers, type Worker, type WorkerType, workerCredentials } from "@/lib/workforce-data"
...
function CredentialBadge({ workerId }: { workerId: string }) {
  ...
  if (cred.type === "ephemeral_oauth") {
    return (
      <span title="Ephemeral OAuth — auto-rotating" className="inline-flex items-center gap-1 rounded-full border border-chart-3/30 bg-chart-3/10 px-1.5 py-0.5 text-xs text-chart-3">
